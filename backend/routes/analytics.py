...
def get_heatmap_data():
    ...
    while current_date <= end_date:
        ...
        count = sum(1 for task in completed_tasks if task.completed_at.date() == current_date)
...
def get_heatmap_data():
    ...
    # Get completion data for the last year
    one_year_ago = datetime.utcnow() - timedelta(days=365)
    
    completions = db.session.query(
        func.DATE(TaskCompletion.completed_at).label('date'),
        func.count(TaskCompletion.id).label('count')
    ).join(
        Task, Task.id == TaskCompletion.task_id
    ).filter(
        Task.user_id == user_id,
        TaskCompletion.completed_at >= one_year_ago
    ).group_by(
    ...
    heatmap_data = [{
        'day': completion.date.strftime('%Y-%m-%d'),
...
def get_productivity_metrics():
    ...
    completions = db.session.query(
        ...
        TaskCompletion.completed_at - Task.created_at
    ).join(
    ...
    estimation_errors = []
    for estimated, actual in completions:
        if actual and estimated:
            actual_minutes = actual.total_seconds() / 60
            error = (actual_minutes - estimated) / estimated * 100  # Percentage error
            estimation_errors.append(abs(error))
    
    avg_estimation_error = np.mean(estimation_errors) if estimation_errors else 0
    
    # Calculate completion rate by day of week
    day_completions = db.session.query(
        extract('dow', TaskCompletion.completed_at).label('day_of_week'),
        func.count(TaskCompletion.id).label('count')
    ).join(
        Task, Task.id == TaskCompletion.task_id
    ).filter(
        ...
        extract('dow', TaskCompletion.completed_at)
    ).all()
    ...
    completion_by_day = [{
        'day': days[int(day)],
        'count': count
    } for day, count in day_completions]
    
    return jsonify({
        'metrics': {
            'avg_estimation_error': round(avg_estimation_error, 1),
            'completion_by_day': completion_by_day,
            'tasks_analyzed': len(completions)
        }
    }), 200
